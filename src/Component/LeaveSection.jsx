      ...
      >
        ...
        <div className="ml-44 bg-blue-50 p-6">
          ...
          {/* <div className="grid grid-cols-4 gap-4 mb-6">
            {["Leave Settings", "Leave Recall", "Leave History", "Relief Officers"].map((item) => (
              <button
                key={item}
                onClick={() => setActiveSection(item.replace(" ", "").toLowerCase())}
                className={`py-4 px-6 rounded-xl text-center transition-colors ${
                  activeSection === item.replace(" ", "").toLowerCase() ? "bg-yellow-500 text-white" : "bg-blue-900 text-white"
                }`}
              >
                {item}
              </button>
          ...
          <div>
            ...
            {activeTab === "" && (
              <div className="bg-blue-900 flex justify-between items-center text-white rounded-3xl pl-10 pr-20 py-20 relative overflow-hidden">
                ...
                <div>
                  <img src={Leave} alt="" className="w-48 h-48 object-cover" />
                </div>
              </div>
            )}
          </div>
